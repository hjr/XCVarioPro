...
int sock_ctrl_t::connect_sta_socket(esp_ip4_addr* ip_info) {
    state = SocketState::HAVE_IP;
    _nr_peers = 0;

    // Connect to AP, e.g. XCV master
    peer_record new_rec;
    memset(&new_rec, 0, sizeof(new_rec));  // alles nullen
    new_rec.clientAddress.sin_family      = AF_INET;
    new_rec.clientAddress.sin_port        = htons(port);   // port muss gesetzt sein!
    new_rec.clientAddress.sin_addr.s_addr = ip_info->addr;

    if (sock_hndl >= 0) {
        shutdown(sock_hndl, SHUT_RDWR);
        close(sock_hndl);
        sock_hndl = -1;
    }
    sock_hndl = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (sock_hndl < 0) {
        ESP_LOGE(FNAME, "failed to allocate socket: %s, port %d", strerror(errno), port);
        return -1;
    }
    if (connect(sock_hndl, (struct sockaddr*)&new_rec.clientAddress, sizeof(new_rec.clientAddress)) < 0) {
        ESP_LOGE(FNAME, "socket connect failed: %s, port %d", strerror(errno), port);
        close(sock_hndl);
        sock_hndl = -1;
        return -1;
    }
    // vTaskDelay(pdMS_TO_TICKS(5000)); // that seems to avoid the select errno 22
    int flag = 1;  // prepare for short telegram transmission
    setsockopt(sock_hndl, IPPROTO_TCP, TCP_NODELAY, (char*)&flag, sizeof(int));

    state = SocketState::CONNECTED;
    is_ap = false;
    _nr_peers = 1;
    peers[0] = new_rec;  // sta has just one peer, eg the master XCVario

    ESP_LOGI(FNAME, "socket %d connected successfully!", sock_hndl);
    return sock_hndl;
}
...
void sock_ctrl_t::removePeer(int idx) {
    if (peers[idx].sock_hndl >= 0) {
        shutdown(peers[idx].sock_hndl, SHUT_RDWR);
        close(peers[idx].sock_hndl);
        peers[idx].sock_hndl = -1;
        peers[idx].retries = 0;
        if (_nr_peers > 0) _nr_peers--;
    }
}

void sock_ctrl_t::removeAllPeers() {
    for (int i = 0; i < MAX_CLIENTS; i++) {
        removePeer(i);
    }
...
void sock_ctrl_t::addPeer(int peer_sock, struct sockaddr_in clientAddress) {
    int idx = nextFreePeer();
    if (idx >= 0) {
        peers[idx].sock_hndl = peer_sock;
        peers[idx].retries = 0;
        peers[idx].clientAddress = clientAddress;
        ESP_LOGI(FNAME, "New client: %d, number of clients: %d", peer_sock, _nr_peers);
    }
    else {
        ESP_LOGW(FNAME, "No free peer slot available for new client on port %d, rejecting connection", port);
...
// Net status helper
void NetState::reset() {
    std::lock_guard<SemaphoreMutex> lock(_m); wifi = WIFI_OFF; nrsock = 0;
}
void NetState::operator++(int) {
    std::lock_guard<SemaphoreMutex> lock(_m); ++nrsock;
}
void NetState::operator--(int) {
    std::lock_guard<SemaphoreMutex> lock(_m); if (nrsock > 0) --nrsock;
}
void NetState::setStaState(StaState s) {
    std::lock_guard<SemaphoreMutex> lock(_m); wifi = s;
}
...
public:
	...
	static void socket_server(void *arg)
	{
        ...
        NetState& state = wifi->state;
        wifi_queue = xQueueCreate(10, sizeof(WifiEvent));
        WifiEvent evt;
        bool run_server = true;

        while (run_server) {
            // The STA state machine is driven by wifi events
            ...
            while (xQueueReceive(wifi_queue, &evt, wait_time) == pdTRUE) {
                ESP_LOGI(FNAME, "Received wifi event: %d", evt.type);
                switch (evt.type) {
                ...
                case WifiEventType::STA_CONNECTED: {
                    ...
                    ESP_LOGI(FNAME, "STA connected");
                    break;
                }
                case WifiEventType::STA_GOT_IP: {
                    state.setStaState(NetState::WIFI_GOT_IP);
                    esp_ip4_addr* evt_ip = (esp_ip4_addr*)&evt.data;
                    ESP_LOGI(FNAME, "STA got ip gateway:" IPSTR, IP2STR(evt_ip));

                    // only from here on sockets are allowed
                    sock_ctrl_t* xcvcl = wifi->_socks[4];
                    if (xcvcl && xcvcl->connect_sta_socket(evt_ip) >= 0) {
                        ESP_LOGI(FNAME, "STA socket connected to master XCVario");
                        state.setStaState(NetState::WIFI_STA_UP);
                        wait_time = 0;
                    }
                ...
                case WifiEventType::STA_DISCONNECTED: {
                    state.setStaState(NetState::WIFI_STARTED);  // bump state back to started
                ...
                if (config && config->sock_hndl >= 0) {
                    ...
                    if (config->is_ap) {
                        for (int i = 0; i < MAX_CLIENTS; i++) {
                            ...
                            if (client_rec.sock_hndl >= 0) {
                                FD_SET(client_rec.sock_hndl, &read_fds);
                                max_fd = std::max(max_fd, client_rec.sock_hndl);
                            }
                        }
                    }
                }
            ...
            // ESP_LOGI(FNAME, "Waiting for socket activity, max_fd: %d", max_fd);
            ...
            for (int n = 0; n < NUM_TCP_PORTS; n++) {
                    ...
                    if (config->is_ap) {
                        ...
                        if (config->_nr_peers < MAX_CLIENTS) {
                            ...
                            if (new_client >= 0) {
                                ...
                                if (free_peer_idx < 0) {
                                    ESP_LOGW(FNAME, "No free peer slot available for new client on port %d, rejecting connection",
                                             config->port);
                                    close(new_client);
                                    continue;
                                }
                                peer_record& new_rec = config->peers[free_peer_idx];
                                new_rec.sock_hndl = new_client;
                                new_rec.retries = 0;
                ...
                if (n == 4) {
                    continue;  // skip the STA socket aöready handled
                }

                for (int i = 0; i < MAX_CLIENTS; i++) {
                    peer_record& client_rec = config->peers[i];
                    if (client_rec.sock_hndl >= 0 && FD_ISSET(client_rec.sock_hndl, &read_fds)) {
                        ESP_LOGI(FNAME, "FD_ISSET socket: connection %d, port %d", client_rec.sock_hndl, config->port);
                        char r[ProtocolItf::MAX_LEN + 1];
                        ssize_t sizeRead = recv(client_rec.sock_hndl, r, ProtocolItf::MAX_LEN - 1, MSG_DONTWAIT);
                        if (sizeRead > 0) {
                            ...
                            ESP_LOGI(FNAME, "Client %d disconnected", client_rec.sock_hndl);
                            config->removePeer(i);
                            continue;
                        }
                    } else {
                    ...
                    if (client_rec.retries > 100) {
                        ESP_LOGW(FNAME, "tcp client %d (port %d) 100retries: %s!", client_rec.sock_hndl, config->port, strerror(errno));
                ...
                if (uxTaskGetStackHighWaterMark(wifi->socket_server_task_pid) < 128) {
                    ESP_LOGW(FNAME, "Warning wifi task stack low: %d bytes, port %d",
                             uxTaskGetStackHighWaterMark(wifi->socket_server_task_pid), config->port);
                }
            }
        }
        vTaskDelete(NULL);
	...
	for (int i=0; i < NUM_TCP_PORTS; i++) {
		if (_socks[i]) {
			for (int c=0; c < MAX_CLIENTS; c++) {
				...
				if (client_rec.sock_hndl >= 0) {
					shutdown(client_rec.sock_hndl, SHUT_RDWR);
					close(client_rec.sock_hndl);
				}
			}
...
void WifiApSta::ConfigureIntf(int port) {
    ...
    if (port >= 8880 && port <= 8884) {
        ...
        initialize_wifi(true, 2, OTA_SSID); // socket server not needed for the OTA webserver
    } else {
...
int WifiApSta::Send(const char* msg, int& len, int port) {
    ...
    if (!socks->is_ap) {
        ...
        {
            peer_record& p_rec = socks->peers[c];
            if (p_rec.sock_hndl >= 0) {
                int num = send(p_rec.sock_hndl, msg, len, MSG_DONTWAIT);
                ...
                if (num == len) {
                    ...
                    p_rec.retries++;
                    ESP_LOGW(FNAME, "tcp send to %d (port: %d) failed: %s, retries: %d", p_rec.sock_hndl, port, strerror(errno), p_rec.retries);
                    if (errno == ECONNRESET || errno == EPIPE || errno == ENOTCONN || p_rec.retries > 3) {
                        ESP_LOGW(FNAME, "Client %d on port %d %s, removing", p_rec.sock_hndl, port, strerror(errno));
                        socks->removePeer(c);
                    }
                }
            }
...
static void wifi_config_sta(const char* staid, const char* appass) {
    ...
    if (staid != nullptr && staid[0] != '\0') {
        ...
        wifi_sta_config.sta.pmf_cfg = {
            .capable  = true,   // PMF supported, we will use it if the AP also supports it
...
static esp_netif_t* wifi_config_ap(uint8_t maxcon, const char* ssid) {
    ...
    wifi_ap_config.ap.pmf_cfg.capable  = false;
