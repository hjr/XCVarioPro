...
void GyroMPU6050::postProcess() {
    ...
    _processed = gyro - _bias_estimator.getRef(); // a corrected measurment
    ...
    bool rest = detectRest() && accSensor->isResting() && (!airborne.get() || abs(omegalp) < Units::deg_to_rad(1.5f));

    // feed the bias filter
    if ( rest ) {
        _bias_estimator.filter(gyro);
    }
